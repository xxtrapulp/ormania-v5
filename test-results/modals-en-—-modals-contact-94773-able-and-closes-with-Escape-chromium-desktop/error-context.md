...
Call log:
  - waiting for locator('button').filter({ hasText: 'Contact Us' }).first()
    - locator resolved to visible <button class="group inline-flex items-center gap-2 px-6 py-3 rounded-full bg-gold text-ink font-medium text-[0.9rem] btn-sheen active:scale-[0.96] transition-transform">…</button>
